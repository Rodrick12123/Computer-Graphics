...
void camGetOrthographic(const camCamera *cam, double proj[4][4]) {
	proj[0][0] = 2/(cam->projection[1] - cam->projection[0]);
    proj[0][1] = 0;
    proj[0][2] = 0;
    proj[0][3] = (-cam->projection[1] - cam->projection[0])/(cam->projection[1] - cam->projection[0]);
    proj[1][0] = 0;
    proj[1][1] = 2/(cam->projection[3] - cam->projection[2]);
    proj[1][2] = 0;
    proj[1][3] =(-cam->projection[3] - cam->projection[2])/(cam->projection[3] - cam->projection[2]);
    proj[2][0] = 0;
    proj[2][1] = 0;
    proj[2][2] = -2/(cam->projection[5] - cam->projection[4]);
    proj[2][3] = (cam->projection[5] + cam->projection[4])/(cam->projection[5] - cam->projection[4]);
    proj[3][0] = 0;
    proj[3][1] = 0;
    proj[3][2] = 0;
    proj[3][3] = 1;
...
void camGetProjectionInverseIsometry(const camCamera *cam, double homog[4][4]) {
	double camInvIsom[4][4]; 
	double proj[4][4];
	isoGetInverseHomogeneous(&cam->isometry, camInvIsom);
	if (cam->projectionType == camORTHOGRAPHIC) {
		camGetOrthographic(cam, proj);
	} else {
		camGetPerspective(cam, proj);
	}
	mat444Multiply(proj, camInvIsom, homog);
}
