...
void isoUntransformPoint(
	...
	double rotationInverse[3][3], pMinusT[3];
	mat33Transpose(iso->rotation, rotationInverse);
	vecSubtract(3, isoP, iso->translation, pMinusT);
	mat331Multiply(rotationInverse, pMinusT, p);
}
...
void isoGetInverseHomogeneous(const isoIsometry *iso, double homogInv[4][4]) {
	double rotationInverse[3][3], transInverse[3], rotTransInv[3];
	mat33Transpose(iso->rotation, rotationInverse);
	vecScale(3, -1, iso->translation, transInverse);
	mat331Multiply(rotationInverse, transInverse, rotTransInv);
	mat44Isometry(rotationInverse, rotTransInv, homogInv);
}
