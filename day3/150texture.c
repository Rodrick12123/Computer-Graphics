...
void texSample(const texTexture *tex, double s, double t, double sample[]) {
    ...
    if (tex->filtering == texNEAREST){
        ...
        double first[tex->texelDim];
        double second[tex->texelDim];
        double firstPLUSsecond[tex->texelDim];
        double third[tex->texelDim];
        double fourth[tex->texelDim];
        double thirdPLUSfourth[tex->texelDim];

        double fracU = u - floor(u);
        double fracV = v - floor(v);

        texGetTexel(tex,(int)floor(u),(int)floor(v),sample);
        vecScale(tex->texelDim,(1 - fracU) * (1 - fracV),sample,first);

        texGetTexel(tex,(int)ceil(u),(int)floor(v),sample);
        vecScale(tex->texelDim,(fracU) * (1 - fracV),sample,second);

        vecAdd(tex->texelDim,first,second,firstPLUSsecond);

        texGetTexel(tex,(int)floor(u),(int)ceil(v),sample);
        vecScale(tex->texelDim,(1 - fracU) * (fracV),sample,third);

        texGetTexel(tex,(int)ceil(u),(int)floor(v),sample);
        vecScale(tex->texelDim,(fracU) * (fracV),sample,fourth);

        vecAdd(tex->texelDim,third,fourth,thirdPLUSfourth);

        vecAdd(tex->texelDim,firstPLUSsecond,thirdPLUSfourth,sample);
    }
}
