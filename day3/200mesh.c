...
void meshRender(
	...
	if(mesh->attrDim == sha->attrDim){
		for (int i = 0; i < mesh->triNum; i++){
			int *trianglei = meshGetTrianglePointer(mesh, i);

			//attr
			double *p1 = meshGetVertexPointer(mesh, trianglei[0]);
			double *p2 = meshGetVertexPointer(mesh, trianglei[1]);
			double *p3 = meshGetVertexPointer(mesh, trianglei[2]);
			double v1[sha->varyDim], v2[sha->varyDim], v3[sha->varyDim];
			
			//vary
			shadeVertex(
			sha->unifDim, unif, sha->attrDim, p1,
			sha->varyDim, v1);	
			shadeVertex(
			sha->unifDim, unif, sha->attrDim, p2,
			sha->varyDim, v2);	
			shadeVertex(
			sha->unifDim, unif, sha->attrDim, p3,
			sha->varyDim, v3);

			triRender(sha, unif, tex, v1, v2, v3);
		}
	}

	else {
		printf("Mesh attrDim does not equal sha attrDim");}	
